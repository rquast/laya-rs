...
|---|---|---|
| `REQUIRED_FILES` | `const REQUIRED_FILES: &[&str]` (18) | `rl_agent_config.json`, `encoder/config.json`, `tokenizer/tokenizer.json`, `tokenizer/tokenizer_config.json`, `model.safetensors` (mirrors website demo `CHECKPOINT_FILES`) |
| `cache_dir()` | (22) | `$XDG_CACHE_HOME` (must be absolute) else `$HOME/.cache`; returns `$base/laya-rs`; None if neither set |
| `variant_cache_dir(v)` | (37) | cache dir + `hf_repo` basename (`laya-typed-decisions` / `laya-multilingual`) |
| `download_variant(v)` | (86) | `create_dir_all(dir)`; per file: skip if `dest.is_file()`; first missing file + `LAYA_OFFLINE` set → `bail!("{file} is missing from {dir} and LAYA_OFFLINE is set; unset it to allow downloading, or pass --model")`; else `fetch_to_file`. Returns dir |
...
- Fully-cached scenario: deterministic offline (LAYA_OFFLINE set + all files present → skip loop, no fetch).
- Partial+offline scenario: deterministic error, names first missing file + `LAYA_OFFLINE` + `--model`.
- Fresh-cache real download: ~300 MB from HF — gated behind `LAYA_TEST_DOWNLOAD` env (skip otherwise), same opt-in pattern as `LAYA_TEST_MODEL`.
