...
Runs the `laya` CLI (ask/answer/train) or depends on the `laya-rs` crate from a Rust service
...
Uses the in-browser playground on the laya-rs website to build choice/score/noul questions against a document and see typed answers instantly, without installing anything
...
**Goals:**
- Domain-adapt the model with `laya train` over a JSONL dataset
