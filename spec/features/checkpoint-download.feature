...
Feature: Download a checkpoint into the local cache
  ...
  #
  # BUSINESS RULES:
  #   1. The five required files (rl_agent_config.json, encoder/config.json, tokenizer/tokenizer.json, tokenizer/tokenizer_config.json, model.safetensors) are downloaded into $XDG_CACHE_HOME/rlcd-rs/<repo-basename> (default ~/.cache/rlcd-rs), one per Hugging Face URL
  ...
  #
  # EXAMPLES:
  #   1. On a fresh cache, requesting the typed-decisions variant fetches all five files from huggingface.co into ~/.cache/rlcd-rs/laya-typed-decisions and returns that directory
  ...
  Background: User Story
    ...
    So that `rlcd ask`/`answer` work with zero manual setup on a fresh machine

  Scenario: A fresh cache downloads all five files
    Given a cache directory without the typed-decisions checkpoint
    When I request the typed-decisions variant download over the network (opt-in via RLCD_TEST_DOWNLOAD; the real checkpoint is several hundred MB)
  ...
  Scenario: A fully cached download touches nothing
    ...
    When I request the variant download with RLCD_OFFLINE set
  ...
  Scenario: Offline with a partial cache is an actionable error
    Given a cache holding only some of the checkpoint files and RLCD_OFFLINE set
    When I request the variant download
    Then it fails with an error naming the first missing file, RLCD_OFFLINE, and --model
