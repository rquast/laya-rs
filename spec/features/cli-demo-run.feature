...
Feature: Built-in demo run against a state body
  ...
  #
  # BUSINESS RULES:
  #   1. With no subcommand, `laya` takes a positional state body (defaulting to "We were billed twice for March. Please refund the duplicate.") and builds a demo state JSON {subject: "Duplicate charge on invoice 4411", body: <state>}
  ...
  #
  # EXAMPLES:
  #   1. A user runs `laya "The server has been down since Tuesday and no one has replied"` and sees the routed checkpoint on stderr plus three answer lines — a department choice with option probabilities, an urgency score with level probabilities, and a churn_risk noul value
  #   2. A user runs `laya` with no arguments at all and the demo runs against the built-in default state body "We were billed twice for March. Please refund the duplicate."
  #
  # ========================================
  Background: User Story
    As a user who just installed laya
    I want to run `laya` with no subcommand to see the decision engine answer a demo question battery against a body of text
  ...
  Scenario: Demo battery prints one line per question type
    ...
    When the user runs `laya "The server has been down since Tuesday and no one has replied"`
  ...
  Scenario: No arguments use the built-in default body
    Given a laya checkpoint is available
    When the user runs `laya` with no arguments at all
