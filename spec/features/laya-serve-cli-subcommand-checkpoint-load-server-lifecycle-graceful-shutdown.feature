...
Feature: `laya serve` CLI subcommand: checkpoint load, server lifecycle, graceful shutdown
  ...
  #
  # BUSINESS RULES:
  #   1. laya serve accepts --model DIR | --model-variant KEY [--models-root DIR] (identical precedence to ask/answer via model_path::resolve) and --host (default 127.0.0.1) / --port (default 8000) / --max-model-len (default: checkpoint max_len) / --max-request-branches (100) / --max-queued (16); --max-model-len is clamped to the checkpoint's native max_len
  ...
  #
  # EXAMPLES:
  #   1. Running `laya serve --port 8000` while another process owns port 8000 fails with a readable bind error before serving anything; running with --max-model-len 4096 against a checkpoint whose native max_len is 1024 silently clamps the effective cap to 1024
  #   2. Running `laya serve --model /path/to/laya-typed-decisions --port 8000` loads the checkpoint, prints `laya serving '/path/to/laya-typed-decisions' on http://127.0.0.1:8000`, serves the endpoints, and on Ctrl-C completes any in-flight forward before exiting 0
  ...
  #   4. `laya serve --max-queued 0` and `laya serve --max-request-branches -1` each exit non-zero immediately with a clap usage error naming the flag and NO checkpoint-load progress output — nothing is loaded and no HTTP is ever served
  #
  # ========================================
  Background: User Story
    As a ops engineer
    I want to launch the server with one laya serve command and checkpoint flags matching the existing subcommands
  ...
  Scenario: Serve loads the checkpoint and serves on the requested port
    ...
    When the user runs `laya serve --model /path/to/laya-typed-decisions --port 8000`
    Then the checkpoint loads with stderr progress, the server prints `laya serving '/path/to/laya-typed-decisions' on http://127.0.0.1:8000`, and /health reports that model name

  Scenario: Port conflicts and load failures fail cleanly before serving
    Given another process already owns port 8000
    When the user runs `laya serve --port 8000`
  ...
  Scenario: The effective sequence cap clamps to the checkpoint's native max_len
    ...
    When the user runs `laya serve --max-model-len 4096`
