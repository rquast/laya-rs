...
Feature: Resolve a checkpoint directory by variant
  """
  src/model_path.rs (native-only): resolve(default_variant_key, explicit, requested_variant, family_root). VARIANTS: typed-decisions (hf convaiinnovations/laya-typed-decisions), multilingual (hf convaiinnovations/laya-multilingual). Order: explicit as-is -> family root subfolder (candidates: hf repo basename then subfolder; must contain model.safetensors) -> download::download_variant (cache $XDG_CACHE_HOME/laya-rs/<repo-basename>, 5 REQUIRED_FILES, skip existing, LAYA_OFFLINE -> bail with 'unset it or pass --model').
  """
  ...
  #   3. Failing both, the variant's cached standalone download ($XDG_CACHE_HOME/laya-rs/<repo-name>) is returned, downloading missing files from Hugging Face first; LAYA_OFFLINE makes a missing file an error instead of a network fetch
  #
  # EXAMPLES:
  #   1. With a family root that contains the variant's subfolder, `laya ask` loads the subfolder checkpoint directly — no cache lookup, no download
  ...
  #   3. When a checkpoint file is missing from the cache and LAYA_OFFLINE is set, resolution fails with a clear error telling the user to unset it or pass --model
  #
  # ========================================
  ...
  Scenario: A missing cache file offline is a clear error
    Given an empty cache with LAYA_OFFLINE set
    When the resolver resolves the variant
    Then it fails with an error that names LAYA_OFFLINE and --model
