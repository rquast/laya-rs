...
Feature: Answer a batch of typed questions via CLI
  ...
  #
  # EXAMPLES:
  #   1. A user runs `rlcd answer input.json answers.json --model /path/to/laya-typed-decisions` where input.json holds a state and three questions (a choice, a score, a noul); the command writes answers.json with one typed answer per question id and prints a per-question timing summary to stderr
  #   2. Running `rlcd answer` on an input file with no `state` field fails with a `missing state` error
  #   3. Running `rlcd answer input.json answers.json --only churn_risk` answers only the question with id `churn_risk` and writes a single-entry answers.json
  #
  # ========================================
  Background: User Story
    As a user running the rlcd CLI
  ...
  Scenario: Answer a mixed batch and write typed answers
    ...
    When the user runs `rlcd answer input.json answers.json --model /path/to/laya-typed-decisions`
  ...
  Scenario: Missing state field is an error
    ...
    When the user runs `rlcd answer input.json answers.json`
  ...
  Scenario: --only restricts the batch to one question
    ...
    When the user runs `rlcd answer input.json answers.json --model /path/to/laya-typed-decisions --only churn_risk`
  ...
  Scenario: Malformed input file is a parse error
    ...
    When the user runs `rlcd answer bad.json answers.json --model /path/to/laya-typed-decisions`
