...
Feature: Ask a single choice question via CLI
  ...
  #   3. `rlcd ask` requires --state and --question; --option is a repeatable flag and each value becomes a choice criterion with no description
  #
  # EXAMPLES:
  #   1. Running `rlcd ask --state "We were billed twice" --question "Which team should handle this?" --option billing --option technical` routes the state, loads the routed checkpoint, and prints the chosen option with its probability distribution
  #   2. Running `rlcd ask --model /path/to/laya-typed-decisions --state "..." --question "..." --option a --option b` loads the given checkpoint directory as-is, skipping routing and models-root resolution
  #
  # ========================================
  ...
  Scenario: Ask a routed choice question and print probabilities
    ...
    When the user runs `rlcd ask --state "<state>" --question "Which team should handle this?" --option billing --option technical`
  ...
  Scenario: Explicit --model bypasses routing and resolution
    ...
    When the user runs `rlcd ask --model /path/to/laya-typed-decisions --state "..." --question "..." --option a --option b`
  ...
  Scenario: Missing required flags produce a usage error
    Given the `rlcd` binary is available
    When the user runs `rlcd ask` without --state or --question
